...
import { coLimitersInitialState } from "./constants";
import { NEW_CO_LIMITERS, EDIT_CO_LIMITERS } from "actions/types";
import { newOredit } from "actions/products";

const CoLimitersFormFields = ({ edit, history, newOredit, product }) => {
  const reducerType = edit ? EDIT_CO_LIMITERS : NEW_CO_LIMITERS;
  const [onChange, onSubmit, formData] = dashboardFormHook(
    coLimitersInitialState,
    reducerType,
    edit,
    history,
    newOredit,
...
export default connect(null, { newOredit })(CoLimitersFormFields);
